...
PART_TWO_SEEDS = [(SEEDS[i], SEEDS[i] + SEEDS[i+1]) for i in range(0, len(SEEDS), 2)]
...
def almanac_part_two():
    global PART_TWO_SEEDS

    mappings = create_mappings()
    seed_ranges = PART_TWO_SEEDS
...
def almanac_brute_force():
    ...
    for (seed_start, delta) in SEEDS:
        ...
        locations.append(almanac_part_one(new_seeds))
...
seed_ranges = SEEDS
part_one_result = almanac_part_one()
part_two_result = almanac_part_two()
time_end = time.time()
